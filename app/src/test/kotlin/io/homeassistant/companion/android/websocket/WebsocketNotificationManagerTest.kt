...
class WebsocketNotificationManagerTest {
    ...
    @Test
    fun `Given an active notification flow completes when leased then subscription is restarted`() = runTest {
        val restartedNotifications = MutableSharedFlow<Map<String, Any>>()
        val server = createServer()
        coEvery { serverManager.getServer(any<Int>()) } returns server
        coEvery { serverManager.webSocketRepository(server.id) } returns webSocketRepository
        coEvery { webSocketRepository.getNotifications() } returnsMany listOf(emptyFlow(), restartedNotifications)
        val manager = WebsocketNotificationManager(serverManager, messagingManager).also {
            it.collectionScope = backgroundScope
        }

        assertNotNull(manager.acquire(ServerManager.SERVER_ID_ACTIVE))
        coVerify(exactly = 1) { webSocketRepository.getNotifications() }

        advanceTimeBy(1_000)
        runCurrent()

        coVerify(exactly = 2) { webSocketRepository.getNotifications() }
    }
